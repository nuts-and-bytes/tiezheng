...
test('totals 统计天数/组数/次数/容量（容量只算重量×次数齐全的组）', () => {
  const r = totals(
    [
      { exerciseId: 'p-bench', sets: [{ weight: 60, reps: 10 }, { weight: 60, reps: 8 }] }, // 600+480
      { exerciseId: 'p-pullup', sets: [{ weight: 0, reps: 10 }, { reps: 12 }] },             // 0 + 无重量
    ],
    ['2026-07-01', '2026-07-02', '2026-07-01'],
  );
  expect(r).toEqual({ days: 2, sets: 4, reps: 40, volumeKg: 1080, moves: 2 });
});
...
test('totals：纯自重也数得出负荷——次数照算，容量诚实归零', () => {
  const r = totals(
    [
      { exerciseId: 'p-pushup', sets: [{ reps: 20 }, { reps: 18 }] }, // 俯卧撑，一次重量都没填
      { exerciseId: 'p-curl', sets: [{ weight: 40 }] },               // 只填重量没填次数：两边都不算
    ],
    ['2026-07-01'],
  );
...
/**
 * 「负荷」那一格摆哪个数——数据页 / 资料页 / 海报三处唯一的梯子。
 *
 * 原来只有两级：有重量 → 容量，否则 → 总次数。可 sanitizeSets 的注释白纸黑字允许
 * 「全空时保留组数——徒手训练允许只记组数不记次数」。这类人的 volumeKg 和 reps 双 0，
 * 于是他练了 48 组，海报上印着「总次数 0」发给朋友。
 * 他的记录里唯一还没被前两格（打卡天数 / 总组数）用掉的真实维度，是动作数。
 */
describe('loadKind（负荷那一格摆哪个数）', () => {
  it('有重量 → 容量', () => {
    expect(loadKind(ITEMS)).toBe('volume');
  });

  it('没重量但记了次数（自重党）→ 总次数', () => {
    expect(
      loadKind([{ date: '2026-07-01', exerciseId: 'p-pullup', sets: [{ weight: 0, reps: 10 }] }]),
    ).toBe('reps');
  });

  it('连次数都不记，只记组数 → 动作数（不是一个假的「总次数 0」）', () => {
    expect(
      loadKind([
        { date: '2026-07-01', exerciseId: 'p-pullup', sets: [{}, {}, {}] },
        { date: '2026-07-01', exerciseId: 'p-pushup', sets: [{}, {}] },
      ]),
    ).toBe('moves');
  });

  // reps: 0 是 sanitizeSets 拦不住的边界（LIMITS.reps.min = 1，但历史数据/直接写库能进来）。
  // 0 次不是一个负荷维度——它和「没记」是同一件事，不该把梯子卡在第二级上显示「总次数 0」。
  it('次数记成 0 等同于没记 → 动作数', () => {
    expect(
...
describe('totals().moves（动作数：去重后的动作条目数）', () => {
  it('同一天同一个动作分两条记，只算一个动作', () => {
    const t = totals(
      [
        { exerciseId: 'p-bench', sets: [{}, {}] },
        { exerciseId: 'p-bench', sets: [{}] },
...
describe('compare', () => {
  ...
  // 动作数要能上第三格，就得跟另外三个指标一样带环比——否则那一格是个没有对照的孤数字，
  // 而这一排大数字存在的全部理由就是「没有对比的数字没有意义」（StatsScreen 原话）。
  it('动作数也带环比', () => {
    const dates = ITEMS.map((i) => i.date);
    const r = compare(
      ITEMS,
