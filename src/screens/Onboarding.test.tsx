...
/** 让 navigator 具备 Web Share files 能力（iOS Safari / 新 Android Chrome 的现实） */
...
async function gotoPoster(user: ReturnType<typeof userEvent.setup>): Promise<HTMLElement> {
  renderOnboarding();
  await user.click(screen.getByRole('button', { name: '开始' }));
  await user.click(screen.getByRole('button', { name: '继续' }));
  return activeScreen();
...
describe('屏 3 的海报承诺必须是真的', () => {
  test('能调起系统分享面板时才说「存进相册」', async () => {
    enableShare();
    const s3 = await gotoPoster(userEvent.setup());

    expect(within(s3).getByText(/存进相册/)).toBeInTheDocument();
  });

  test('不支持 Web Share 的浏览器（桌面 / 微信内嵌）只会下载 PNG —— 不许承诺相册', async () => {
    // jsdom 默认没有 navigator.share，正是这类环境
    const s3 = await gotoPoster(userEvent.setup());

    expect(within(s3).queryByText(/存进相册/)).not.toBeInTheDocument();
    expect(within(s3).getByText(/下载/)).toBeInTheDocument();
  });

  test('样例海报的月份跟着当前月走 —— 写死的月份下个月就成了过期广告', async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true, now: new Date('2027-03-05T10:00:00') });
    const user = userEvent.setup({ advanceTimers: vi.advanceTimersByTime });

    const s3 = await gotoPoster(user);

    expect(within(s3).getByText(/2027\.03/)).toBeInTheDocument();
  });
});
