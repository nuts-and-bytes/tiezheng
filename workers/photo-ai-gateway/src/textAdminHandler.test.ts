...
describe('internal text admin configuration firewall', () => {
  test('loads the admin vars in their fixed fail-closed defaults without opening existing gateways', () => {
    const vars = runtimeEnv as unknown as Record<string, unknown>;
    expect(vars.TEXT_AI_ADMIN_ENABLED).toBe('false');
    expect(vars.TEXT_AI_CONNECTIVITY_PROBE_ENABLED).toBe('false');
...
describe('internal DeepSeek connectivity probe', () => {
  test.each([undefined, 'false', 'TRUE', ' true', 'true '])(
    'fails closed for non-exact TEXT_AI_CONNECTIVITY_PROBE_ENABLED=%s',
    async (flag) => {
      const { gatewayEnv, getByName } = coordinatorHarness();
      Object.assign(gatewayEnv as unknown as Record<string, unknown>, {
        TEXT_AI_CONNECTIVITY_PROBE_ENABLED: flag,
        DEEPSEEK_API_KEY: 'deepseek-test-key',
      });
      const fetcher = vi.fn();
      const response = await handleTextAdminRequest(
        adminRequest(JSON.stringify({
          ...ADMIN_BODY,
          operation: 'probe-deepseek-connectivity',
        })),
        gatewayEnv,
        { now: () => BASE_NOW, fetcher } as unknown as TextAdminDependencies,
      );
  ...
  test('requests only the official model list and returns a redacted HTTP class', async () => {
    const { gatewayEnv, getByName, applyTextAdminOperation } = coordinatorHarness();
    Object.assign(gatewayEnv as unknown as Record<string, unknown>, {
      TEXT_AI_CONNECTIVITY_PROBE_ENABLED: 'true',
      DEEPSEEK_API_KEY: 'deepseek-test-key',
    });
    const providerResponse = new Response('private provider response', { status: 200 });
    ...
    const response = await handleTextAdminRequest(
      ...
      } as unknown as TextAdminDependencies,
    );
  ...
  test('uses a fixed deadline and reports only timeout', async () => {
    const { gatewayEnv } = coordinatorHarness();
    Object.assign(gatewayEnv as unknown as Record<string, unknown>, {
      TEXT_AI_CONNECTIVITY_PROBE_ENABLED: 'true',
      DEEPSEEK_API_KEY: 'deepseek-test-key',
    ...
    const setDeadline = vi.fn((callback: () => void, delay: number) => {
    ...
    expect(setDeadline).toHaveBeenCalledTimes(1);
    expect(clearDeadline).toHaveBeenCalledWith(7);
  });
  ...
  test.each([
    ...
    [401, 'http-4xx'],
    [503, 'http-5xx'],
  ] as const)('reduces provider HTTP %i to %s', async (status, connectivity) => {
    const { gatewayEnv } = coordinatorHarness();
    Object.assign(gatewayEnv as unknown as Record<string, unknown>, {
      TEXT_AI_CONNECTIVITY_PROBE_ENABLED: 'true',
      DEEPSEEK_API_KEY: 'deepseek-test-key',
  ...
  test('contains rejected fetch details in one fixed category', async () => {
    const { gatewayEnv } = coordinatorHarness();
    Object.assign(gatewayEnv as unknown as Record<string, unknown>, {
      TEXT_AI_CONNECTIVITY_PROBE_ENABLED: 'true',
      DEEPSEEK_API_KEY: 'deepseek-test-key',
